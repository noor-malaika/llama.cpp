...
int32_t ggml_et_prefetch_dest() {
    static const int32_t dest = [] {
        const char * v = getenv("GGML_ET_PREFETCH_DEST");
        if (!v || !v[0]) {
            return 1;  // L2, matching the original hardcoded behaviour
        }
        return atoi(v) == 0 ? 0 : 1;
    }();
    return dest;
...
bool ggml_et_op_mul_mat_ffn_glu(ggml_backend_et_device_context* dev_ctx,
    ...
    params.prefetch_dest = ggml_et_prefetch_dest();
...
bool ggml_et_op_set_rows_pair(ggml_backend_et_device_context* dev_ctx,
    ...
    // Both copies are distributed over the same threads, so size the launch to
    // whichever half needs more of them.
    const int64_t units_a = ggml_et_cacheline_units(first->src[0]);
    const int64_t units_b = ggml_et_cacheline_units(second->src[0]);
    const uint64_t shire_mask = ggml_et_shire_mask_for(units_a > units_b ? units_a : units_b);

    bool kernel_result = ggml_et_launch_kernel(dev_ctx, "set_rows_f32_pair",
                                               &params, sizeof(params), shire_mask);

    ET_PERF_END_EXT("SET_ROWS_PAIR", "set_rows_f32_pair", first, "paired_with=%s",
                    second->name);
    return kernel_result;
}
